...
class WalletFile extends EventEmitter {
    constructor(options)
    {
        ...
        this.queue.on('progress', (progress) => {
            self.emit('sync_status', progress);
        })
    ...
    async ReceivedScriptHashStatus(s, status)
    {
        ...
        if (status && status != prevStatus)
        {
            ...
            this.queue.add(this, this.SyncScriptHash, [s], true)
        }
    }
    ...
    async SyncScriptHash(scripthash)
    {
        ...
        while (true) {
            ...
            for (var j in newHistory.history) {
                ...
                this.queue.add(this, this.GetTxKeys, [hash, height, true])
            }
    ...
    {
        let utxos = (await this.db.asyncFind({outPoint: {$exists: true}, spentIn: '', xnav: xnav?1:0, cold: onlyCold?1:0}))
    ...
    async GetBalance()
    {
        ...
        return {nav: {confirmed: navConfirmed.toNumber(), pending: navPending.toNumber()}, xnav: {confirmed: xNavConfirmed.toNumber(), pending: xNavPending.toNumber()}, cold: {confirmed: coldConfirmed.toNumber(), pending: coldPending.toNumber()}}
    }
                    ...
                    if (hid) {
                        ...
                        if ((await this.db.asyncFind({key: hashId})).length) {
                            if (blsct.RecoverBLSCTOutput(prevOut, this.mvk)) {
                                ...
                                mine = true;
                                let newOutput = await this.AddOutput(`${input.prevTxId}:${input.outputIndex}`, prevOut)
                    ...
                    if (hid) {
                        ...
                        if ((await this.db.asyncFind({key: hashId})).length) {
                            if (blsct.RecoverBLSCTOutput(out, this.mvk)) {
                                mine = true;
                                let newOutput = await this.AddOutput(`${tx.txid}:${i}`, out)
    ...
    async AddStakingAddress(pk) {
        if (pk instanceof bitcore.Address || (typeof pk === 'string' && !bitcore.util.js.isHexa(pk)))
            return await this.AddStakingAddress(bitcore.Address(pk).toObject().hash)

        if (pk instanceof Buffer)
            return await this.AddStakingAddress(pk.toString('hex'))
        ...
        if (input.script)
        {
            ...
            else if (script.isColdStakingOutP2PKH() || script.isColdStakingV2Out())
            {
                if (script.isColdStakingOutP2PKH()) {
                    ...
                    this.AddStakingAddress(stakingPk)
    ...
    async GetTxKeys (hash, height, useCache = true) {
        ...
        if (isMine)
        {
            this.queue.add(this, this.GetTx, [hash, inMine, height], true)
        }
