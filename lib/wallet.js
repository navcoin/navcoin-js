...
class WalletFile extends EventEmitter {
  ...
  async GetAllAddresses() {
    ...
    for (let i in receiving) {
      ...
      ret.spending[address.address] = {
        balances: await this.GetBalance(address.address),
        used: address.used,
      };

      let label = await this.db.GetLabel(address);

      if (label) ret.spending[address.address].label = label;
    }
    ...
    for (let j in staking) {
      ...
      if (label) ret.staking[address.address].label = label;
    }
  ...
  async SyncScriptHash(scripthash) {
    ...
    while (true) {
      ...
      for (let j in newHistory.history) {
        ...
        await this.db.AddScriptHashHistory(
          scripthash,
          newHistory.history[j].tx_hash,
          newHistory.history[j].height,
          false
        );
