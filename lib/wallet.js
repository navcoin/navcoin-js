...
class WalletFile extends EventEmitter {
    ...
    static async ListWallets() {
        return await this.db.ListWallets();
    }

    static async RemoveWallet(filename) {
        return await this.db.RemoveWallet(filename);
    }
    ...
    async xNavCreateSubaddress(sk, acct=0) {
        ...
        await this.db.AddKey(hashId, [acct, index], AddressTypes.XNAV, blsct.KeysToAddress(viewKey, spendKey).toString(), false, false, acct+'/'+index)
    }
