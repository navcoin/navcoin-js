...
class WalletFile extends EventEmitter {
  ...
  async UpdateDotNav(name, subdomain, key, value, spendingPassword) {
    ...
    let privk = new Buffer(await this.DeriveSpendingKeyFromStringHash("name/", name, spendingPassword);
    let k = blsct.SkToPubKey(privk).toString('hex'));

    if (k.serializeToHexStr() != nameResolve["_key"])
      throw new Error("You don't own the name.");

    let vData = Buffer.concat([new Buffer([first ? 7 : 8, 0, 0, 0]), new Buffer([name.length]), new Buffer(name, 'utf-8'), new Buffer([48]), new Buffer(k.serialize()), new Buffer([subdomain.length]), new Buffer(subdomain, 'utf-8'), new Buffer([key.length]), new Buffer(key, 'utf-8'), new Buffer([value.length]), new Buffer(value, 'utf-8'), privk]);
