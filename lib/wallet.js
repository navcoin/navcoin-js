...
class WalletFile extends EventEmitter {
  ...
  async GetBalance(address) {
    ...
    for (let u in utxos) {
      ...
      if (
        ...
        let tokId =
            txObj.outputs[prevOut].tokenId.toString("hex") +
            String(txObj.outputs[prevOut].tokenNftId);
        if (
            tokId ==
          ...
          if (pending) {
            if (!tokPending[tokId]) tokPending[tokId] = 0;
            tokPending[tokId] = tokPending[tokId] + utxo.amount;
          } else {
            if (!tokConfirmed[tokId]) tokConfirmed[tokId] = 0;
            tokConfirmed[tokId] = tokConfirmed[tokId] + utxo.amount;
          }
        }
      } else {
        if (
  ...
  async GetTx(hash, inMine, height, requestInputs = true) {
    ...
    if (requestInputs) {
      ...
      if (mustNotify && mine) {
        if (deltaXNav != 0 || memos.out.length) {
          this.emit("new_tx", {
            txid: tx.txid,
            amount: deltaXNav,
            type: "xnav",
  ...
  async DeriveSpendingKeyFromStringHash(prefix, name, spendingPassword) {
    ...
    let ret = new Buffer(msk.serialize());
    ...
    for (let i = 0; i < 8; i++) {
      let index =
          ((hash[i * 4] << 24) |
              ...
              hash[i * 4 + 3]) >>>
          0;
      ret = blsct.DeriveChildSK(ret, index);
    }
    return ret;
  }
  ...
  async RegisterDotNav(name, spendingPassword) {
    ...
    let vData = Buffer.concat([new Buffer([6, 0, 0, 0]), bufferHash]);

    return await this.xNavCreateTransaction(bitcore.Script.fromHex("6ac1"), consensus[23].value, "", spendingPassword, true, new Buffer(new Uint8Array(32)), -1, vData);
  }

  async UpdateDotNav(name, subdomain, key, value, spendingPassword) {
    let first = false;
    let size = 0;
    let nameResolve = {};
    ...
    try {
      ...
      {
        for (var key in nameResolve) {
          size += key.length + nameResolve[key].length;
        }
      }
    } catch(e) {
      first = true;
    ...
    let k = blsct.SkToPubKey(new Buffer(this.DeriveSpendingKeyFromStringHash("name/", name, spendingPassword)).toString('hex'));

    if (k.serializeToHexStr() != nameResolve["_key"])
      throw new Error("You don't own the name.");

    let sh = name + k.serializeToHexStr();
    let nameHash = bitcore.crypto.Hash.sha256sha256(
        Buffer.concat([new Buffer([sh.length]), new Buffer(sh, "utf-8")])
    );
    ...
    let vData = Buffer.concat([new Buffer([first ? 7 : 8, 0, 0, 0]), new Buffer([name.length]), new Buffer(name, 'utf-8'), new Buffer(k.serialize()), new Buffer([subdomain.length]), new Buffer(subdomain, 'utf-8'), new Buffer([key.length]), new Buffer(key, 'utf-8'), new Buffer([value.length]), new Buffer(value, 'utf-8')]);

    return await this.xNavCreateTransaction(bitcore.Script.fromHex("6ac1"), consensus[23].desc, "", spendingPassword, true, new Buffer(new Uint8Array(32)), -1, vData);
  }
