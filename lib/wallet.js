...
class WalletFile extends EventEmitter {
  ...
  async Load(options) {
    ...
    if (!this.db)
      throw new Error("DB did not load.")
    ...
    if (this.newWallet && (this.type === "navcash" || this.type == "navcoin-js-v1")) {
      let pool =
  ...
  async GetUtxos(
    ...
    if (
        typeof address === "string" && !bitcore.util.js.isHexa(address)
    )
      return await this.GetUtxos(type, bitcore.Address(address), tokenId, tokenNftId);
      ...
      for (let i in tx.tx.outputs) {
        ...
        if (out.isCt()) {
          ...
          if (hid) {
            let hashId = new Buffer(hid).toString("hex");
            if (await this.db.GetKey(hashId)) {
              if (blsct.RecoverBLSCTOutput(out, this.mvk, undefined, undefined, undefined, out.tokenId, out.tokenNftId)) {
                mine = true;
                let newOutput = await this.AddOutput(
                  `${tx.txid}:${i}`,
                  out,
  ...
  async DeriveSpendingKeyFromStringHash(prefix, name, spendingPassword)
  {
    if (
        typeof name === "string")
    {
      name = bitcore.crypto.Hash.sha256sha256(Buffer.concat([new Buffer([name.length]), new Buffer(name, 'utf-8')]))
    }

    let sh = prefix + name.reverse().toString('hex');
    let hash = bitcore.crypto.Hash.sha256sha256(Buffer.concat([new Buffer([sh.length]), new Buffer(sh, 'utf-8')]))
        ...
        tx.to(
            bitcore.Address.fromBuffers(
                [
                  ...
                  bitcore.Address((await this.NavReceivingAddresses())[0].address).toBuffer().slice(1),
                ],
                this.network,
                "coldstaking"
            ),
