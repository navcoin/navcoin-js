...
class WalletFile extends EventEmitter {
    constructor(options)
    {
        ...
        this.db = new Db(options.file, secret, options.adapter);
        ...
        if (!(await this.db.GetValue('masterPubKey')))
        {
            ...
            if (options.bootstrap)
            {
                ...
                for (let i in bootstrap) {
                    bootstrap[i]._id = 'txKeys_'+bootstrap[i].txidkeys
                    bootstrap[i].doc_type = 'txKeys'
    ...
    async NavGetPrivateKeys (spendingPassword, address) {
        ...
        for (let i in list) {
            list[i].privateKey = (await this.GetPrivateKey(list[i]._id.split("_")[1], spendingPassword)).toWIF()
            delete list[i].value
        }
    ...
    async Sync(staking = undefined)
    {
        ...
        if (!staking) {
            let pending = await this.db.GetTxs()
    ...
    async GetUtxos(type = OutputTypes.NAV|OutputTypes.STAKED)
    {
        ...
        for (let u in utxos) {
            ...
            let outpoint = utxo._id.split('_')[1].split(':')
    ...
    async GetBalance()
    {
        ...
        {
            let utxo = utxos[u];
            let prevHash = utxo._id.split('_')[1].split(':')[0];
            ...
            {
                if (utxo.type & OutputTypes.STAKED)
                {
                    if (pending)
                ...
                else if (utxo.type & OutputTypes.NAV)
                {
                    if (pending)
    ...
    async AddOutput(outpoint, out)
    {
        ...
        try {
            await this.db.AddUtxo(outpoint, out.toBufferWriter().toBuffer().toString('hex'), '', amount, label, type)

            if (!out.isCt()) {
                await this.db.UseNavAddress(out.script.toAddress(this.network))
            } else {
                let hashId = new Buffer(blsct.GetHashId(out, this.mvk)).toString('hex')
