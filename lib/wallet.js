...
class WalletFile extends EventEmitter {
      ...
      for (let i in tx.tx.outputs) {
        ...
        if (out.vData[0] == 6) {
          try {
            ...
            if (out.vData.slice(4,36) == bufferHash)
              await this.AddName(decryptedName.toString());
          } catch(e) {
  ...
  async AddName(name) {
    try {
      await this.db.AddName(name);
      this.emit('new_name', name)
      return true;
    }
    catch(e)
    {
  ...
  async UpdateName(name, subdomain, key, value, spendingPassword) {
    ...
    let fee = Math.floor(size/consensus[26].value)*consensus[27].value;
    ...
    return await this.xNavCreateTransaction(bitcore.Script.fromHex("6ac1"), fee, "", spendingPassword, false, new Buffer(new Uint8Array(32)), -1, vData, privk);
  }
