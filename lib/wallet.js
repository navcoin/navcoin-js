...
class WalletFile extends EventEmitter {
  ...
  async RegisterDotNav(name, spendingPassword) {
    ...
    let key = blsct.SkToPubKey(new Buffer(this.DeriveSpendingKeyFromStringHash("name/", name, spendingPassword)).toString('hex'));
  ...
  async UpdateDotNav(name, subdomain, key, value, spendingPassword) {
    ...
    try {
      name = await this.ResolveName(name);
      ...
      {
        for (var key in name) {
          size += key.length + name[key].length;
        }
      }
    } catch(e) {
      first = true;
    ...
    if (k.serializeToHexStr() != name["_key"])
