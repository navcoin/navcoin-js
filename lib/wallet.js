...
class WalletFile extends EventEmitter {
    constructor(options)
    {
        ...
        this.queue.on('progress', (progress, pending) => {
            self.emit('sync_status', progress, pending);
        })
    ...
    async SyncScriptHash(scripthash)
    {
        ...
        while (true) {
            ...
            for (var j in newHistory.history) {
                ...
                try {
                    this.db.asyncUpdate({scriptHashHistory: scripthash, tx_hash: newHistory.history[j].tx_hash}, {scriptHashHistory: scripthash, tx_hash: newHistory.history[j].tx_hash, height: newHistory.history[j].height, fetched: false}, {upsert: true});
                } catch(e) { }
            ...
            else if (script.isColdStakingOutP2PKH() || script.isColdStakingV2Out())
            {
                ...
                if ((await this.db.asyncFind({key: hashId})).length)
                {
                    if (script.isColdStakingOutP2PKH()) {
                        ...
                        await this.AddStakingAddress(stakingPk)
