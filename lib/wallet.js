...
class WalletFile extends EventEmitter {
  ...
  async GetTokenInfo(id) {
    let ret = await this.db.GetToken(id);

    if (!ret || !ret.name) {
      try {
        let token = await this.client.blockchain_token_getInfo(id);
  ...
  async tokenCreateTransaction(
    ...
    if (!utxos.length) throw new Error("No available xNAV outputs");
    if (!utxosTok.length) throw new Error("No available Token outputs");
  ...
  async CreateToken(name, token_code, token_supply, spendingPassword) {
    ...
    let vData = Buffer.concat([
      ...
      new Buffer([token_code.length]),
      new Buffer(token_code, 'utf-8'),
      new Buffer(bitcore.crypto.Blsct.bytesArray(token_supply).reverse())
    ]);

    return await this.xNavCreateTransaction(bitcore.Script.fromHex("6ac1"), 0, "", spendingPassword, false, new Buffer(new Uint8Array(32)), -1, vData, derived);
  }
  ...
  async MintToken(id, dest, amount, spendingPassword) {
    ...
    return await this.tokenCreateTransaction(dest, amount, "", spendingPassword, new Buffer(id, 'hex'), -1, vData, derived);
  }
