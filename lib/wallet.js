...
class WalletFile extends EventEmitter {
    constructor(options)
    {
        ...
        this.host = 'electrum-testnet2.nav.community'
        this.port = 40004
        this.proto = 'wss'
    ...
    async NavCreateAddress(sk, change=false) {
        ...
        await this.db.asyncUpdate({key: hashId}, {key: hashId, value: pkStr, navAddress: true, path: path, address: bitcore.Address(pk, this.network).toString(), used: false, change: change, index: index}, {upsert: true});
    }
    ...
    async SendTransaction(txs) {
        ...
        else
        {
            try
            {
                return await this.client.blockchain_transaction_broadcast(txs)
            }
            catch(e)
            {
                console.error(`error sending tx: ${e}`)
...
module.exports.xNavBootstrapTestnet = require('./xnav_bootstrap_testnet')