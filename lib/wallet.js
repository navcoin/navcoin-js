...
class WalletFile extends EventEmitter {
    ...
    async xNavCreateSubaddress(sk, acct=0) {
        let masterViewKey = blsct.mcl.deserializeHexStrToFr(this.mvk);
    ...
    async GetHistory()
    {
        let list = (await this.db.asyncFind({wallettxid: {$exists: true}}, [['sort', { height: -1, pos: -1 }]]))
    ...
    async GetUtxos(xnav = false)
    {
        ...
        for (var u in utxos) {
            ...
            if (!pending)
            {
                let out = bitcore.Transaction.Output.fromObject(utxo.out)
