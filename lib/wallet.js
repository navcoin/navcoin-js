...
class WalletFile extends EventEmitter {
    ...
    async GetPoolSize(type) {
        return await this.db.asyncCount({type: type, index: {$exists: true}, used: false})
    }
    ...
    async Load(options) {
        ...
        if (creationTipDb.length) {
            ...
            await this.db.asyncUpdate({creationTip: {$exists: true}}, {creationTip: this.creationTip}, {upsert: true})
        }
    ...
    async xNavReceivingAddresses (all=true) {
        let list = all ? await this.db.asyncFind({type: AddressTypes.XNAV, index: {$exists: true}}, [['sort', { index: 1 }]]) : await this.db.asyncFind({type: AddressTypes.XNAV, used: false, index: {$exists: true}}, [['sort', { index: 1 }]]);

        return list;
    }

    async NavReceivingAddresses (all=true) {
        let list = all ? await this.db.asyncFind({type: AddressTypes.NAV, path: {$exists: true}}, [['sort', { index: 1 }]]) : await this.db.asyncFind({type: AddressTypes.NAV, used: false, path: {$exists: true}}, [['sort', { index: 1 }]]);

        return list;
    }

    async NavGetPrivateKeys (spendingPassword, address) {
        let list = address ? await this.db.asyncFind({type: AddressTypes.NAV, address: address, path: {$exists: true}}) : await this.db.asyncFind({type: AddressTypes.NAV, path: {$exists: true}});
    ...
    async xNavCreateSubaddress(sk, acct=0) {
        ...
        await this.db.asyncUpdate({key: hashId}, {key: hashId, value: [acct, index], type: AddressTypes.XNAV, address: blsct.KeysToAddress(viewKey, spendKey).toString(), used: false, index: index}, {upsert: true});
    }
    ...
    async NavCreateAddress(sk, change=false) {
        ...
        await this.db.asyncUpdate({key: hashId}, {key: hashId, value: pkStr, type: AddressTypes.NAV, path: path, address: addrStr, used: false, change: change, index: index}, {upsert: true});
    ...
    async ImportPrivateKey(privK, key)
    {
        ...
        await this.db.asyncUpdate({key: hashId}, {key: hashId, value: pkStr, type: AddressTypes.NAV, path: path, address: bitcore.Address(pk, this.network).toString(), used: false, change: false}, {upsert: true});
    ...
    async GetScriptHashes(stakingAddress = undefined) {
        ...
        let addresses = await this.db.asyncFind({path: {$exists: true}});
    ...
    async AddOutput(outpoint, out)
    {
        ...
        try {
            await this.db.asyncInsert({outPoint: outpoint, out: out.toBufferWriter().toBuffer().toString('hex'), spentIn: '', amount: amount, label: label, type: type})

            if (!out.isCt()) {
                await this.db.asyncUpdate({address: out.script.toAddress(this.network)}, {$set: {used: true}})
            } else {
                let hashid = new Buffer(blsct.GetHashId(out, this.mvk)).toString('hex')
                await this.db.asyncUpdate({key: hashid}, {$set: {used: true}})
            }
    ...
    async xNavCreateTransaction(dest, amount, memo, spendingPassword, subtractFee=true, type = OutputTypes.NAV|OutputTypes.STAKED)
    {
        if (amount <= 0)
        ...
        let utx = await this.GetUtxos(type)
        let utxos = [];
    ...
    async NavCreateTransaction(dest, amount, memo, spendingPassword, subtractFee=true, fee=100000, ret = {fee:0,tx:[]}, selectxnav = false)
    {
        ...
        let utxos = await this.GetUtxos(OutputTypes.XNAV);
