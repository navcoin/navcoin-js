...
class WalletFile extends EventEmitter {
  ...
  async GetBalance(address) {
    ...
    for (let u in utxos) {
      ...
      if (
        ...
        let tokId =
            ...
            tokenNftId: txObj.outputs[prevOut].tokenNftId};
        if (
            tokId.tokenId ==
    ...
    for (let i in tokConfirmed) {
      if (i.tokenNftId == -1) {
        if (!ret.tokens[i.tokenId])
        {
          ret.tokens[i.tokenId] = {};

          let info = this.GetTokenInfo(i.tokenId);
          ret.tokens[i.tokenId].name = info.name;
          ret.tokens[i.tokenId].code = info.code;
          ret.tokens[i.tokenId].supply = info.supply;
        }
        ret.tokens[i.token_id].confirmed = tokConfirmed[i];
      }
      else
      {
        if (!ret.nfts[i.tokenId])
        {
          ret.nfts[i.tokenId] = {};

          let info = this.GetTokenInfo(i.tokenId);
          ret.nfts[i.tokenId].name = info.name;
          ret.nfts[i.tokenId].scheme = info.code;
          ret.nfts[i.tokenId].supply = info.supply;
          ret.nfts[i.tokenId].confirmed = {};
        }
        let nftInfo = this.GetNftInfo(i.tokenId, i.tokenNftId);

        ret.nfts[i.tokenId].confirmed[i.tokenNftId] = nftInfo.metadata;
      }
    }
    ...
    for (let i in tokPending) {
      if (i.tokenNftId == -1) {
        ...
        ret.tokens[i.token_id].pending = tokPending[i];
      }
      else
      {
        if (!ret.nfts[i.tokenId])
        {
          ...
          ret.nfts[i.tokenId].scheme = info.code;
          ret.nfts[i.tokenId].supply = info.supply;
          ret.nfts[i.tokenId].pending = {};
        }
        let nftInfo = this.GetNftInfo(i.tokenId, i.tokenNftId);

        ret.nfts[i.tokenId].pending[i.tokenNftId] = nftInfo.metadata;
      }
    }
    ...
    if (requestInputs) {
      ...
      for (let i in tx.tx.outputs) {
        ...
        if ((out.vData[0] == 7 || out.vData[0] == 8) && tx.height > -0) {
          ...
          try {
            let tokenPubKey = out.vData.slice(5,5+out.vData[4]).toString("hex");
            console.log(`created token ${tokenPubKey}`)
          }
          catch(e) {
            console.log(e);
          }
  ...
  async tokenCreateTransaction(
    ...
    tokenId = new Buffer(tokenId, "hex").reverse();
  ...
  async CreateToken(name, token_code, token_supply, spendingPassword) {
    ...
    ret.token_id = new Buffer(bitcore.crypto.Hash.sha256sha256(key.serialize())).toString('hex');
  ...
  async MintToken(id, dest, amount, spendingPassword) {
    ...
    if (token.key != key.serializeToHexStr())
