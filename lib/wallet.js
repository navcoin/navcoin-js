...
class WalletFile extends EventEmitter {
    ...
    async Load(options) {
        ...
        if (!(await this.db.GetValue('masterPubKey')))
        {
            ...
            {
                let bootstrap = options.bootstrap[this.network] ? options.bootstrap[this.network] : options.bootstrap;
        ...
        {
            this.AddStakingAddress("NfLgDYL4C3KKXDS8tLRAFM7spvLykV8v9A", undefined, false)
        }
    ...
    async GetAllAddresses () {
        let ret = {spending: {}, staking: {}};

        let receiving = await this.db.GetNavReceivingAddresses(true);

        for (let i in receiving) {
            let address = receiving[i];
            ret.spending[address.address] = { balances: await this.GetBalance(address.address), used: address.used }
        }
        ...
        for (let j in staking) {
            ...
            ret.staking[address.address] = { staking: (await this.GetBalance(address.address)).staked }
        }
    ...
    async NavGetPrivateKeys (spendingPassword, address) {
        let list = (address ? await this.db.GetNavAddress(address) : await this.db.GetNavReceivingAddresses(true));

        for (let i in list) {
            list[i].privateKey = (await this.GetPrivateKey(list[i].hash, spendingPassword)).toWIF()
            delete list[i].value
        }
    ...
    async GetMasterKey(key, password) {
        ...
        if (bitcore.HDPrivateKey(privK).hdPublicKey.toString() !== bitcore.HDPublicKey(pubK).toString())
            return undefined;
    ...
    async GetMasterSpendKey(key) {
        ...
        if (!blsct.mcl.deserializeHexStrToG1(pubK).isEqual(blsct.mcl.mul(blsct.G(), blsct.mcl.deserializeHexStrToFr(privK))))
            return undefined;
    ...
    async xNavCreateSubaddress(sk, acct=0) {
        ...
        await this.db.AddKey(hashId, [acct, index], AddressTypes.XNAV, blsct.KeysToAddress(viewKey, spendKey).toString(), false, false, acct+'/'+parseInt(index))
    }
    ...
    async NavCreateAddress(sk, change=false) {
        ...
        let dbLastIndex = await this.db.GetCounter(change?'NavChange':'Nav');
        ...
        await this.db.UpdateCounter(change?'NavChange':'Nav', index+1)
        await this.db.AddKey(hashId, privK.toString(), AddressTypes.NAV, addrStr, false, change, path, sk)
    ...
    async ImportPrivateKey(privK, key)
    {
        ...
        await this.db.AddKey(hashId, privK.toString(), AddressTypes.NAV, bitcore.Address(pk, this.network).toString(), false, false, path, key)
    ...
    async SetMasterKey(masterkey, key) {
        if (await this.db.GetMasterKey(key))
            return false;
        ...
        await this.db.AddMasterKey('xNavSpend', masterSpendKey.serializeToHexStr(), key)
        await this.db.SetValue('masterViewKey', masterViewKey.serializeToHexStr())
        await this.db.SetValue('masterSpendPubKey', masterSpendPubKey.serializeToHexStr())
        await this.db.SetValue('masterViewPubKey', masterViewPubKey.serializeToHexStr())
    ...
    async SyncScriptHash(scripthash)
    {
        ...
        while (true) {
            ...
            await this.db.CleanScriptHashHistory(scripthash, 0, Math.max(1, currentLastHeight-10));
            ...
            for (let j in newHistory.history) {
                ...
                await this.db.AddScriptHashHistory(scripthash, newHistory.history[j].tx_hash, newHistory.history[j].height, false)
    ...
    async GetBalance(address)
    {
        ...
        for (let u in utxos)
        {
            ...
            if ((tx.pos < 2 && (tip-tx.height) < 120) || tx.height <= 0 || !tx.height || !tx.pos)
                pending = true;

            if (utxo.type & OutputTypes.XNAV)
            {
                if (pending)
            ...
            {
                if (utxo.type & OutputTypes.STAKED && (!address || utxo.stakingPk == address))
                {
                    if (pending)
                ...
                else if (utxo.type & OutputTypes.NAV && (!address || utxo.spendingPk == address))
                {
                    if (pending)
                ...
                if (utxo.type & OutputTypes.VOTING && (!address || utxo.votingPk == address))
                {
                    if (pending)
                        votingPending = votingPending.add(new bitcore.crypto.BN(utxo.amount));
                    else
                        votingConfirmed = votingConfirmed.add(new bitcore.crypto.BN(utxo.amount));
                }
            }
        }

        return {
            nav: {confirmed: navConfirmed.toNumber(), pending: navPending.toNumber()},
            xnav: {confirmed: xNavConfirmed.toNumber(), pending: xNavPending.toNumber()},
            staked: {confirmed: coldConfirmed.toNumber(), pending: coldPending.toNumber()},
            voting: {confirmed: votingConfirmed.toNumber(), pending: votingPending.toNumber()},
        }
    }
    ...
    async AddOutput(outpoint, out)
    {
        ...
        if (out.isCt())
            type |= OutputTypes.XNAV;
        else {
            if (isCold)
                type |= OutputTypes.STAKED;
            else
        ...
        try {
            ...
            if (out.script.isColdStakingOutP2PKH()) {
                ...
                spendingPk = ripemd160(sha256(out.script.getPublicKey())).toString('hex')
            } else if (out.script.isPublicKeyHashOut()) {
                spendingPk = out.script.getPublicKeyHash().toString('hex');
            }

            await this.db.AddUtxo(outpoint, out.toBufferWriter().toBuffer().toString('hex'), '', amount, label, type, spendingPk, stakingPk, votingPk)

            if (!out.isCt()) {
                await this.db.UseNavAddress(out.script.toAddress(this.network).toString())
            } else {
                let hashId = new Buffer(blsct.GetHashId(out, this.mvk)).toString('hex')
    ...
    async GetTx (hash, inMine, height, requestInputs = true)
    {
        ...
        if (tx.height != prevHeight) {
            if (tx.height)
                await this.db.SetTxHeight(hash, tx.height, tx.pos);
            mustNotify = true;
        }
        ...
        if (requestInputs) {
            ...
            if (mustNotify && mine)
            {
                if (deltaXNav != 0) {
                    ...
                    await this.db.AddWalletTx(tx.txid, 'xnav', deltaXNav, tx.height > 0, tx.height, tx.pos, tx.tx.time, memos)
                }
                if (deltaNav != 0) {
                    this.emit('new_tx', {
                        txid: tx.txid,
                    ...
                    await this.db.AddWalletTx(tx.txid, 'nav', deltaNav, tx.height > 0, tx.height, tx.pos, tx.tx.time)
                }
                if (deltaCold != 0) {
                    this.emit('new_tx', {
                        txid: tx.txid,
                    ...
                    await this.db.AddWalletTx(tx.txid, 'cold_staking', deltaCold, tx.height > 0, tx.height, tx.pos, tx.tx.time)
                }
            }
        }
    ...
    async AddStakingAddress(pk, pk2, sync=false) {
        if (pk instanceof bitcore.Address || (typeof pk === 'string' && !bitcore.util.js.isHexa(pk)))
            return await this.AddStakingAddress(bitcore.Address(pk).toObject().hash, pk2, sync)

        if (pk2 instanceof bitcore.Address || (typeof pk2 === 'string' && !bitcore.util.js.isHexa(pk2)))
            return await this.AddStakingAddress(pk, bitcore.Address(pk2).toObject().hash, sync)
    ...
    async IsMine (input) {
        if (input.script)
        {
            ...
            {
                let hashId = new Buffer(script.getPublicKeyHash()).toString('hex')
                let hashIdVoting = new Buffer(script.getVotingPublicKeyHash()).toString('hex')

                if (await this.db.GetKey(hashId) || await this.db.GetKey(hashIdVoting))
                {
                    if (script.isColdStakingOutP2PKH()) {
                        let stakingPk = script.getStakingPublicKeyHash()
                        await this.AddStakingAddress(stakingPk, undefined, true)
    ...
    async NavCreateTransaction(dest, amount, memo, spendingPassword, subtractFee=true, fee=100000, ret = {fee:0,tx:[]}, selectxnav = false, type = OutputTypes.NAV|OutputTypes.STAKED)
    {
        if (amount <= 0)
            throw new TypeError("Amount must be greater than 0")

        if (!(dest instanceof bitcore.Address))
            return await this.NavCreateTransaction(new bitcore.Address(dest), amount, memo, spendingPassword, subtractFee, fee, ret, selectxnav, type);
