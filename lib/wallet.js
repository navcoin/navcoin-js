...
class WalletFile extends EventEmitter {
    ...
    async Load(options) {
        ...
        {
            for (var i in options.bootstrap)
            {
                let item = options.bootstrap[i];
                await this.db.asyncUpdate({txidkeys: item.txidkeys}, item, {upsert: true})
            }
        }
    ...
    async NavCreateTransaction(dest, amount, memo, spendingPassword, subtractFee=true, fee=100000)
    {
        if (!(dest instanceof bitcore.Address))
            return await this.NavSend(new bitcore.Address(dest), amount, memo, spendingPassword, subtractFee);
