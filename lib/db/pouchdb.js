...
module.exports = class extends EventEmitter  {
    ...
    static async RemoveWallet(filename) {
        try {
            await new PouchDB(filename).destroy()
            return true;
        } catch (e) {
            return false
        }
    ...
    async MarkAsFetched(hash) {
        ...
        {
            ret[i].fetched = true
            await this.Upsert(ret[i])
        }
    }

    async GetWalletHistory() {
        let walletTxs = (await this.db.allDocs({
            startkey: 'walletTx',
            endkey: 'walletTx\uffff',
            include_docs: true
        })).rows.map((e) => e.doc)

        let unconfirmed = walletTxs.filter((e) => e.height <= 0)

        unconfirmed.forEach((element) => {
            walletTxs.splice(walletTxs.indexOf(element), 1)
        })
        ...
        unconfirmed.sort(sortFun);

        walletTxs.sort(sortFun);

        return unconfirmed.concat(walletTxs);
    }
