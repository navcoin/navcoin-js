...
if (!PouchDB.on) {
    ...
    PouchDB.plugin(require('pouchdb-adapter-node-websql').default);
}
else
{
...
module.exports = class extends EventEmitter  {
    ...
    async AddMasterKey(type, value, password) {
        ...
        await this.Upsert({doc_type: 'masterKey', _id: 'masterKey_'+type, value: value});
    }
    ...
    async MarkAsFetched(hash) {
        ...
        if (ret.docs.length)
        {
            ret.docs[0].fetched = true
            await this.Upsert(ret.docs[0])
        }
    }
