...
module.exports = class extends EventEmitter {
  ...
  async GetPoolSize(type) {
    ...
    return await this.db.keys.where({ type: type, used: 0 }).count();
  }

  async GetMasterKey(key, password) {
    if (!this.db) return;

    let dbFind = await this.db.encryptedSettings.get({
      key: "masterKey_" + key,
    });
  ...
  async AddMasterKey(type, value, password) {
    ...
    try {
      await this.db.encryptedSettings.put({
        key: "masterKey_" + type,
        value: value,
      });
      return true;
  ...
  async UpdateCounter(index, value) {
    ...
    await this.db.settings.put({ key: "counter_" + index, value: value });
  }
  ...
  async SetStatusForScriptHash(s, st) {
    ...
    await this.db.statuses.put({ scriptHash: s, status: st });
  }
  ...
  async ZapWalletTxes() {
    ...
    let types = ["statuses", "scriptHistories", "outPoints", "walletTxs"];
  ...
  async GetPendingTxs(downloaded = 0) {
    ...
    return await this.db.scriptHistories.toArray();
  }
  ...
  async AddScriptHashHistory(scriptHash, hash, height, fetched) {
    ...
    await this.db.scriptHistories.put({
  ...
  async AddLabel(address, name) {
    ...
    await this.db.labels.put({ address: address, name: name });
  }

  async GetLabel(address) {
    let label = await this.db.labels.get(address);
